...
"""
BACKTEST SERIO 15M CON ZONA DI NO-TRADE

- Usa i modelli già addestrati (XGB + LGB) e la soglia thr_15m_<symbol>.txt
- Calcola p_final = max(p_xgb, p_lgb)
- Costruisce i ritorni LONG/SHORT a 15 minuti dalla barra di INGRESSO
- Motore sequenziale:
    * un solo trade alla volta
    * chiusura dopo HORIZON barre
    * possibilità di avere zona di NO-TRADE:
...
HORIZON = 15          # 15 barre da 1m -> 15 minuti
FEE = 0.0005          # 0.05%

SYMBOLS = ["BTCUSDT", "ETHUSDT", "SOLUSDT", "AVAXUSDT", "XRPUSDT"]

# k per simbolo (come nel train)
K_MAP = {
    "BTCUSDT": 0.6,
    "ETHUSDT": 0.6,
    "SOLUSDT": 1.0,
...
CONF_LEVELS: List[Tuple[float, float]] = [
    (0.55, 0.45),
    (0.60, 0.40),
    (0.65, 0.35),
    (0.70, 0.30),
...
# ================================================================
def load_models_and_threshold(symbol: str):
    ...
    if not os.path.exists(xgb_path) or not os.path.exists(lgb_path):
        raise FileNotFoundError(f"Modelli non trovati per {symbol} in {MODEL_DIR}/")
...
def build_features(symbol: str):
    ...
    # stesso k del training
    k = K_MAP.get(symbol, 0.8)
    df_feat = add_features_pro(df_raw, horizon=HORIZON, k=k)
...
# PREDIZIONI ENSEMBLE
# ================================================================
def generate_predictions(df_feat: pd.DataFrame, xgb, lgb) -> pd.DataFrame:
    df = df_feat.copy()
...
# ================================================================
def build_backtest_df(df_raw: pd.DataFrame, df_pred: pd.DataFrame) -> pd.DataFrame:
    # prezzi
...
# ================================================================
# MOTORE SEQUENZIALE CON ZONA DI NO-TRADE
# ================================================================
def backtest_sequential(df_bt: pd.DataFrame, thr_high: float, thr_low: float) -> Dict:
    """
    - Se non in trade:
        ...
        * altrimenti              -> no-trade
    - Se in trade:
        * chiude dopo HORIZON barre (ritorno calcolato dalla barra di ingresso)
    """
    in_trade = False
    entry_idx = None
    direction = None
...
def backtest_symbol(symbol: str):
    print("\n====================================================")
    print(f"   BACKTEST SERIO 15M — {symbol}")
    print("====================================================")

    xgb, lgb, thr_train = load_models_and_threshold(symbol)
    ...
    df_bt = build_backtest_df(df_raw, df_pred)

    if len(df_bt) == 0:
        print(f"[{symbol}] Nessun dato valido per il backtest.")
        return

    print(f"[{symbol}] Barre disponibili per backtest: {len(df_bt)}")
    print(f"[{symbol}] Soglia training (ensemble): {thr_train:.3f}")

    # ------------------------------------------------------------
    # 1) CONFIGURAZIONE BASE: ALL-IN (nessuna no-trade zone)
    #    p >= thr_train → LONG, p <= thr_train → SHORT
    # ------------------------------------------------------------
    res_base = backtest_sequential(df_bt, thr_high=thr_train, thr_low=thr_train)
    ...
    for hi, lo in CONF_LEVELS:
        res = backtest_sequential(df_bt, thr_high=hi, thr_low=lo)
        conf_results.append(res)

    if conf_results:
        # best PnL
        best_pnl = max(conf_results, key=lambda r: r["pnl_total"])
        # best winrate con almeno 50 trade, altrimenti best winrate comunque
        best_wr_candidates = [r for r in conf_results if r["trades"] >= 50]
